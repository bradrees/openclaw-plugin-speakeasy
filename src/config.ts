...
import type { ResolvedSpeakeasyAccount, SpeakeasyAccountConfig } from "./types.js";
...
export function parseSpeakeasyAccount(
  raw: unknown,
  accountId = DEFAULT_ACCOUNT_ID
): ResolvedSpeakeasyAccount {
  const parsed = accountSchema.parse(raw) satisfies SpeakeasyAccountConfig;
  return {
    accountId,
    ...parsed
  };
...
export function validateSpeakeasyAccount(
  raw: unknown,
  accountId = DEFAULT_ACCOUNT_ID
): { ok: true; value: ResolvedSpeakeasyAccount } | { ok: false; errors: string[] } {
...
export function readSpeakeasyChannelSection(cfg: OpenClawConfigLike): SpeakeasyChannelSection {
  const rawSection = cfg.channels?.speakeasy ?? {};
  return speakeasyChannelSchema.parse(rawSection);
}

export function resolveSpeakeasyAccount(
  cfg: OpenClawConfigLike,
  accountId?: string | null
): ResolvedSpeakeasyAccount {
  const section = readSpeakeasyChannelSection(cfg);
  const resolvedAccountId =
    accountId && section.accounts[accountId] ? accountId : Object.keys(section.accounts)[0] ?? DEFAULT_ACCOUNT_ID;
...
export function listSpeakeasyAccountIds(cfg: OpenClawConfigLike): string[] {
  return Object.keys(readSpeakeasyChannelSection(cfg).accounts);
}

export function writeSpeakeasyAccount(
  cfg: OpenClawConfigLike,
  ...
  const storedAccount = (channels.speakeasy as { accounts: Record<string, Record<string, unknown>> }).accounts[
    account.accountId
  ];
...
export function buildConfigValidationError(errors: string[]): Error {
